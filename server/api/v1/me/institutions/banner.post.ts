import { isHemocioneCdnUrl } from "~/utils/institutionImage";
import {
...
const MAX_MULTIPART_REQUEST_BYTES =
  MAX_INSTITUTION_BANNER_BYTES + MULTIPART_OVERHEAD_BYTES;
...
export default defineEventHandler(async (event) => {
  ...
  if (!contentType.toLowerCase().startsWith("multipart/form-data")) {
    throw createError({
      statusCode: 422,
      statusMessage: "O banner deve ser enviado como multipart/form-data",
    });
  }
  ...
  if (
    ...
    throw createError({
      statusCode: 413,
      statusMessage: "O banner deve ter no máximo 4 MB",
    });
  }
  ...
  if (!imagePart?.data || !imagePart.filename) {
    throw createError({
      statusCode: 422,
      statusMessage: "Selecione um arquivo de banner",
    });
  }

  let validation;
  try {
    validation = validateInstitutionBannerData(
      imagePart.data,
      imagePart.type || ""
    );
  } catch (error: any) {
    throw createError({
      statusCode: error.message.includes("MB") ? 413 : 422,
  ...
  if (
    ...
    throw createError({
      statusCode: 502,
      statusMessage: "O serviço de upload não retornou a URL do banner",
    });
  }
