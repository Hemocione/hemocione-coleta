...
export default defineEventHandler(async (event) => {
  ...
  if (!authToken) {
    throw createError({
      statusCode: 401,
      statusMessage:
        "Authentication required to respond to a counter proposal",
    });
  }
  ...
  if (!parsed.success) {
    throw createError({
      statusCode: 400,
      statusMessage: "Dados inválidos para responder à contraproposta",
    });
  }

  try {
    const updatedRequest = await respondToCounterProposal(
    ...
    throw createError({
      ...
      statusMessage:
        error.message || "Não foi possível responder à contraproposta",
    });
  }
});
