...
describe("validateInstitutionBannerData", () => {
  it("accepts a wide PNG within the size limit", () => {
    expect(validateInstitutionBannerData(pngData(1600, 400), "image/png")).toEqual({
      mimeType: "image/png",
      width: 1600,
      height: 400,
    });
  ...
  it("accepts a rectangular JPEG", () => {
    expect(validateInstitutionBannerData(jpegData(1200, 500), "image/jpeg")).toEqual({
      mimeType: "image/jpeg",
      width: 1200,
      height: 500,
    });
  ...
  it("accepts the MIME types used by institution banners", () => {
    expect(INSTITUTION_BANNER_MIME_TYPES).toEqual(["image/jpeg", "image/png"]);
  });

  it("rejects an image larger than the banner size limit", () => {
    expect(() =>
      validateInstitutionBannerData(
        pngData(1, 1, MAX_INSTITUTION_BANNER_BYTES + 1),
        "image/png"
      )
    ).toThrow("MB");
  });
  ...
  it("rejects a MIME type that does not match the image bytes", () => {
    expect(() =>
      validateInstitutionBannerData(pngData(1, 1), "image/jpeg")
    ).toThrow("tipo JPEG");
  });
  ...
  it("rejects truncated PNG and JPEG files", () => {
    ...
    expect(() =>
      validateInstitutionBannerData(png.subarray(0, -12), "image/png")
    ).toThrow("dimensões");
    expect(() =>
      validateInstitutionBannerData(jpeg.subarray(0, -2), "image/jpeg")
    ).toThrow("dimensões");
  });
});
